...
class kp(nn.Module):
    def __init__(
        ...
        self.tl_whs = nn.ModuleList([
            make_regr_layer(cnv_dim, curr_dim, 2) for _ in range(nstack)
        ])
        self.br_whs = nn.ModuleList([
            make_regr_layer(cnv_dim, curr_dim, 2) for _ in range(nstack)
        ])
    ...
    def _train(self, *xs):
        ...
        for ind, layer in enumerate(layers):
            ...
            tl_wh, br_wh = tl_wh_(tl_cnv), br_wh_(br_cnv)
            tl_tag  = _tranpose_and_gather_feat(tl_tag, tl_inds)
            br_tag  = _tranpose_and_gather_feat(br_tag, br_inds)
            tl_regr = _tranpose_and_gather_feat(tl_regr, tl_inds)
            br_regr = _tranpose_and_gather_feat(br_regr, br_inds)
            tl_wh = _tranpose_and_gather_feat(tl_wh, tl_inds)
            br_wh = _tranpose_and_gather_feat(br_wh, br_inds)

            outs += [tl_heat, br_heat, tl_tag, br_tag, tl_regr, br_regr, tl_wh, br_wh]
...
class AELoss(nn.Module):
    ...
    def forward(self, outs, targets):
        ...
        tl_whs = outs[6::stride]
        br_whs = outs[7::stride]
        ...
        wh_loss = 0
        for tl_wh, br_wh in zip(tl_whs, br_whs):
            wh_loss += self.regr_loss(tl_wh, gt_tl_wh, gt_mask)
            wh_loss += self.regr_loss(br_wh, gt_br_wh, gt_mask)
        wh_loss = self.regr_weight * wh_loss


        loss = (focal_loss + pull_loss + push_loss + regr_loss + wh_loss) / len(tl_heats)
