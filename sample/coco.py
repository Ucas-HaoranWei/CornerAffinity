...
def kp_detection(db, k_ind, data_aug, debug):
    ...
    tl_whs    = np.zeros((batch_size, max_tag_len, 2), dtype=np.float32)
    br_whs    = np.zeros((batch_size, max_tag_len, 2), dtype=np.float32)
    ...
    for b_ind in range(batch_size):
        ...
        for ind, detection in enumerate(detections):
            ...
            tl_whs[b_ind, tag_ind, :] = [math.log(xbr - xtl + 1), math.log(ybr - ytl + 1)]
            br_whs[b_ind, tag_ind, :] = [math.log(xbr - xtl + 1), math.log(ybr - ytl + 1)]
            tag_lens[b_ind] += 1
    ...
    tl_whs = torch.from_numpy(tl_whs)
    br_whs = torch.from_numpy(br_whs)
    tag_masks   = torch.from_numpy(tag_masks)

    return {
        "xs": [images, tl_tags, br_tags],
        "ys": [tl_heatmaps, br_heatmaps, tag_masks, tl_regrs, br_regrs, tl_whs, br_whs]
    }, k_ind
