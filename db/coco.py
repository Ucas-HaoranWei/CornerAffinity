...
class MSCOCO(DETECTION):
    def __init__(self, db_config, split):
        ...
        # self._label_file = os.path.join(self._label_dir, "CityPerson_{}.json")
        # self._label_file = os.path.join(self._label_dir, "box-AirKP_{}.json")
        self._label_file = os.path.join(self._label_dir, "plane-ucasAOD_{}.json")
        ...
        self._data = "coco"
        self._mean = np.array([0.37496743, 0.37341896, 0.34453407], dtype=np.float32)
        self._std  = np.array([0.20108865, 0.19421127, 0.19242503], dtype=np.float32)
        self._eig_val = np.array([0.11348428, 0.00120516, 0.00049268], dtype=np.float32)
        self._eig_vec = np.array([
            [-0.59246377, -0.62955779,  0.50263672],
            [-0.57391571, -0.10800666, -0.81176063],
            [-0.56533834,  0.76940987,  0.29732307]
        ], dtype=np.float32)
        ...
        # self._cache_file = os.path.join(cache_dir, "CityPerson_coco_{}.pkl".format(self._dataset))
        # self._cache_file = os.path.join(cache_dir, "box-AirKP_coco_{}.pkl".format(self._dataset))
        self._cache_file = os.path.join(cache_dir, "plane-ucasAOD_coco_{}.pkl".format(self._dataset))
        self._load_data()
