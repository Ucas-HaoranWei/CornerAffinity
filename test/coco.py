...
def kp_detection(db, nnet, result_dir, debug=False, decode_func=kp_decode):
    ...
    for ind in tqdm(range(0, num_images), ncols=80, desc="locating kps"):
        ...
        if True:
            ...
            # # todo 写入真值
            # gt_bboxes = []
            # gts = db._detections[image_id][:, :4]
            # for bb in gts:
            #     x1, x2, y1, y2 = [bb[0], bb[2], bb[1], bb[3]]
            #     gt_box = [x1, y1, x1, y2, x2, y2, x2, y1]
            #     gt_box = list(map(int, gt_box[:]))
            #     gt_bboxes.append(gt_box)
            # color = (0, 255, 0)
            # image_gt = cv2.imread(image_file)
            # for gt_box in gt_bboxes:
            #     cv2.line(image_gt, (gt_box[0], gt_box[1]), (gt_box[2], gt_box[3]), color, 2)
            #     cv2.line(image_gt, (gt_box[4], gt_box[5]), (gt_box[2], gt_box[3]), color, 2)
            #     cv2.line(image_gt, (gt_box[4], gt_box[5]), (gt_box[6], gt_box[7]), color, 2)
            #     cv2.line(image_gt, (gt_box[0], gt_box[1]), (gt_box[6], gt_box[7]), color, 2)
            # debug_file = os.path.join('./results/CornerNet_CP/val_gt/', "{}_gt.jpg".format(image_id[:-4]))
            # cv2.imwrite(debug_file, image_gt)
            ...
            debug_file = os.path.join(debug_dir, "{}.jpg".format(image_id[:-4]))
